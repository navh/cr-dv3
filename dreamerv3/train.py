...
def make_replay(
  ...
  if config.replay == 'uniform' or is_eval:
    ...
    replay = embodied.replay.CountBasedReplay(length, size, directory, hyper=config.replay_hyper)
  elif config.replay == 'adversarial':
...
def make_env(config, **overrides):
  ...
  ctor = {
      ...
      'cdmc': 'embodied.envs.cdmc:CDMC',
      'ddmc': 'embodied.envs.ddmc:DDMC',
  }[suite]
